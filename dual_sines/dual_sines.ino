...
void draw_sine_reverse(int height[], int freq, int shift, int calcDelay){
    ...
    for (int x = 63 ; x > 0; x--){ //generate waveform
        ...
        delay(calcDelay); // delay for drawing effect
    }
}

void draw_sine(int freq, int shift, int calcDelay){
...
void draw_dual_sines()
{
    ...
    //int delayL = (-0.006*randFreqL) + 101.8;

    int randFreqR = random(300, 13000);
    int randShiftR = random(0, 10);
    //int delayR = (-0.006*randFreqR) + 101.8;
    ...
    for (int x = 0 ; x < 31; x++){ // do x < 63 to have them run one after the other instead of simultaneously
        ...
        matrix.drawPixel(x_adj_l, display_ly[x_adj_l], colorL); // use x_adj_l because it isn't adjusted by the random shift = simultaneous drawing still

        // right to left wave: reverse scale and shifted to 0:31
        int x_adj_r = (-1*x) + 31;

        display_rx[x_adj_r] = x_adj_r-randShiftR;
        display_ry[x_adj_r] = sine_scale(sine_quantize(sine_compand(randFreqR), x_adj_r, 0));
        
        matrix.drawPixel(x_adj_r, display_ry[x_adj_r], colorR); // use x_adj_r because it isn't adjusted by the random shift
